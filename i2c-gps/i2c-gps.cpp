...
int i2cgps::init(void) {
	...
	write(gpiofd, "59", 2);
	close(gpiofd);
	gpiofd = open("/sys/class/gpio/gpio60/direction", O_RDWR);
	write(gpiofd, "low", 3);
	close(gpiofd);
	gpiofd = open("/sys/class/gpio/gpio57/direction", O_RDWR);
	write(gpiofd, "in", 2);
	close(gpiofd);
	gpiofd = open("/sys/class/gpio/gpio59/direction", O_RDWR);
	write(gpiofd, "in", 2);
	close(gpiofd);
	gpiofd = open("/sys/class/gpio/gpio57/value", O_RDWR);
	write(gpiofd, "1", 1);
	close(gpiofd);
	gpiofd = open("/sys/class/gpio/gpio59/value", O_RDWR);
	write(gpiofd, "1", 1);
	close(gpiofd);
